...
router.get("/", async (request, response) => {
  ...
  try {
    ...
    response
      .status(404)
      .json({ sucess: false, error: "Erro ao retornar usuario" });
  }
});
...
router.post("/query", async (request, response) => {
  ...
  if (row.length > 0) {
    ...
    response.status(404).json({
      sucess: false,
      error: "Não foi encontrado nenhuma reserva com este CPF",
    });
  }
});
  ...
  if (CPF && name && lastName && phone && amountPeoples && date) {
    ...
    if (row !== undefined) {
      response.status(400).json({
        sucess: false,
        error: "Ja existe uma reserva com este CPF nesta data",
      });
    } else {
      try {
        const insertReserve =
          "INSERT INTO reserve (CPF, name,  lastName, phone , amountPeoples, date) VALUES (?,?,?,?,?,?)";
        await database.run(insertReserve, [
          CPF,
          name,
          lastName,
          phone,
        ...
        response
          .status(200)
          .json({ sucess: "Reserva realizada com sucesso!", error: false });
      } catch (e) {
        response
          .status(404)
          .json({ sucess: false, error: "Não foi possivel cadastrar" });
      }
    }
  } else
    response
      .status(400)
      .json({ sucess: false, error: "Campos nao preenchidos" });
});
...
router.delete("/:id", async (request, response) => {
  ...
  if (idReserve == undefined)
    response
      .status(404)
      .json({ sucess: false, error: "Nenhuma reserva encontrada" });
  try {
    const deleteReserve = "DELETE FROM reserve WHERE id = ?";
    await database.run(deleteReserve, idReserve);
    response
      .status(200)
      .json({ sucess: "Reserva deletada com sucesso!", error: false });
  } catch (e) {
...
router.patch("/:id", async (request, response) => {
  ...
  if (reserves == undefined) {
    response
      .status(404)
      .json({ sucess: false, error: "Reserva não existente" });
  }
  /* if(!cpfCheck(CPF)) {
  ...
  try {
    const updateReserve =
      "UPDATE reserve SET CPF = ?, name = ?, lastName = ?, phone = ?, amountPeoples = ?, date = ? WHERE id = ?";
    await database.run(updateReserve, [
      CPF,
      name,
      lastName,
      phone,
    ...
    response
      .status(404)
      .json({ sucess: false, erro: "Não foi possivel alterar" });
  }
});
